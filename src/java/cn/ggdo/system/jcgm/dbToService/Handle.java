...
public class Handle {
	...
	public boolean setNameUntil() {
		try {
			...
			NameUntil.setServiceName(className);
			NameUntil.setServicePackageName(packageName);
			NameUntil.setServicePathName(outputPath);
			return true;
		} catch (Exception e) {
			return false;
		}
	...
	public boolean writeGenerate() {
		try {
			...
			map.put("PACKAGE_NAME", NameUntil.getServicePackageName());
			//模板的路径
			String templatePath = fu.readPropertiesValue("PathTemplate") + "/";
			//输出的路径
			String outputPath = NameUntil.getServicePathName();
